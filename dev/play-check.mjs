// Plays the real game in headless Chromium: two trainers, two teams, the
// arena picker, then a whole 3D battle to the end (with a switch and a phone
...
// The move buttons: every button in the battle except fullscreen (⤢) and the winner screen's PLAY AGAIN.
const moveButtons = () => page.locator('button').filter({ hasText: /\S/ }).filter({ hasNotText: '⤢' }).filter({ hasNotText: 'PLAY AGAIN' });
...
try {
  await step('game loads', async () => {
    ...
    await page.locator('text=Choose Your Trainer!').first().waitFor({ timeout: 30000 });
  });
  ...
  await step('teams: Pikachu, Charizard, Gyarados vs Ditto, Snorlax, Mew', async () => {
    for (const n of ['Pikachu', 'Charizard', 'Gyarados']) await page.locator(`img[alt="${n}"]`).click();
    await page.locator("text=Misty's Turn").click();
    for (const n of ['Ditto', 'Snorlax', 'Mew']) await page.locator(`img[alt="${n}"]`).click();
    await page.locator('text=BATTLE!').click();
  });
  if (no3d) {
    await step('no arena picker without 3D', async () => {
      await page.waitForTimeout(500);
      if (await page.locator('text=Choose the Battlefield!').count()) throw new Error('arena picker shown although 3D is unavailable');
    });
  } else {
